...
4. **分镜提示词**：切分生视频 Prompt；本镜参考图最多 10 张，顺序即图一…图十；生成时写入图注，调序后可规则刷新标注。「一键发送到豆包」由仓库 `extension/` 里的 Chrome 插件读取 `data-prompt` / `data-images`，打开豆包生视频页并填入。安装见 `extension/README.md`。
